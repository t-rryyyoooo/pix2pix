...
import sys
sys.path.append("..")
from utils.patchGenerator.patch2DGenerator import Patch2DGenerator
...
class Patch2DCreater():
    ...
    def setGenerator(self):
        ...
        self.input_generator  = Patch2DGenerator(
                                    self.input_image_array,  
                                    patch_size = self.input_patch_size,
                                    slide      = self.slide,
                                    axis       = self.axis
                                    )
        self.target_generator = Patch2DGenerator(
                                    self.target_image_array, 
                                    patch_size = self.target_patch_size,
                                    slide      = self.slide,
                                    axis       = self.axis
                                    )
        self.mask_generator   = Patch2DGenerator(
                                    self.mask_image_array,
                                    patch_size = self.target_patch_size,
                                    slide      = self.slide,
                                    axis       = self.axis
