...
class ImageSlicer():
    def __init__(self, input_image, target_image, input_patch_size=None, target_patch_size=None, plane_size=None, slide=None, axis=0, mask_image=None):
        ...
        Parameters: 
            ...
            plane_size (list)         -- If patch_sizes are None and plane_size exists, pad or crop images to plane_axis perpendicular axis and slice it.
        ...
        org_plane_size = self.getPlaneSize(self.input_image_array.shape, axis)
        if plane_size is None:
            plane_size = org_plane_size

        """ If plane_size exists, pad or crop input and target image array. """
        self.diff  = plane_size - org_plane_size
        self.diff  = np.insert(self.diff, self.axis, 0)
        lower_size = (abs(self.diff) // 2).tolist()
        upper_size = ((abs(self.diff) + 1) // 2).tolist()
        if (self.diff < 0).any():
            self.input_image_array  = croppingForNumpy(
                                        self.input_image_array,
                                        lower_size,
                                        upper_size
                                        )
            self.target_image_array = croppingForNumpy(
                                        self.target_image_array,
                                        lower_size,
                                        upper_size
                                        )
            self.mask_image_array   = croppingForNumpy(
                                        self.mask_image_array,
                                        lower_size,
                                        upper_size
                                        )

        elif (self.diff > 0).any():
    ...
    def outputRestoredImage(self):
        ...
        upper_size = ((abs(self.diff) + 1) // 2).tolist()
        if (self.diff < 0).any():
            self.predicted_array = paddingForNumpy(
                                    self.predicted_array,
                                    lower_size,
                                    upper_size
                                    )
